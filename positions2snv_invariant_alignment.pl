...
sub usage
{
	...
	"\t--reference-file:  Reference file (only used when --include-invariant-positions is used)\n".
...
sub construct_snv_invariant_alignment
{
}
...
# create a copy of all reference genome files for each strain
	...
	elsif ($status ne 'valid')
	{
		...
		{		
			...
			for (my $i = 0; $i < @dna; $i++)
			{
				...
				if ($dna[$i] eq '-')
				{
					substr($$seq_data,$pos-1,1) = 'N'; # replace those positions filtered by coverage with N
				}
				else
				{
					substr($$seq_data,$pos-1,1) = $dna[$i];
				}
			}
		}
	}
