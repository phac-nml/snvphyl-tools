...
sub updateMatrixCsv
{
    ...
    open(my $revisedMatrixCsv, '>', $output_dir.'/revisedMatrix.csv') or croak "ERROR: Could not open the output file: $!";
        ...
        if($_->is_Leaf()){
            ...
            foreach( $input_taxa_tree->get_nodes) {
                if($_->is_Leaf()){
                    ...
                    if($hashResult){ print $revisedMatrixCsv $hashResult."\t" };
                }
            }
            print $revisedMatrixCsv "\n";          
        }
...
sub branchLengthToSNP
{
    ...
    open(my $inputPhy, "<", $inputPhyFile) or croak "ERROR: Could not open input phylogeny.phy file.";
    my $input = <$inputPhy>;
...
my $job_out = tempdir('rearrange_snp_matrixXXXXXX', CLEANUP => (not $keep_tmp), DIR => $output_dir) or croak "Could not create temp directory";
...
print $treeout $parentheses_removed;#$tree->to_newick();
close($treeout);
exit;
=head1 NAME
