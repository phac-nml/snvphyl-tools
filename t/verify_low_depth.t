...
my $mapping_dir = "$script_dir/low_depth";
my $mapping_bin = "perl $script_dir/../bin/verify_low_depth.pl";
my ($command);
...
ok(countLines($create_dir."/2.txt") == 74, "The correct number of regions are output when default values are used.");
...
system("$mapping_bin -c 8 --max-depth 7 --bam bam1=$mapping_dir/input/sample1.bam --bam bam2=$mapping_dir/input/sample2.bam --bam bam3=$mapping_dir/input/sample3.bam --bam bam4=$mapping_dir/input/sample4.bam > ".$create_dir."/3.txt 2>&1");
ok(countLines($create_dir."/3.txt") == 133, "The correct number of regions are logged when min_depth altered.");
