...
my $output = `perl $script_dir/../lib/reporter.pl --output=reporter.json --step=bam_quality_data --bam bam1=reporter/sample1.bam --bam bam2=reporter/sample2.bam --bam bam3=reporter/sample3.bam`;
#is_valid_json($output, "The bam quality stats are being generated properly.");

$output = `perl $script_dir/../lib/reporter.pl --step=record_filter_stats --output=reporter.json --pseudo=/../t/reporter/pseudoalign-positions1.tsv --json=reporter.json`;

#is_valid_json($output, "The filter stats data is being generated properly.");

$output = `perl $script_dir/../lib/reporter.pl --step=record_reference_info --output=reporter.json --json=reporter.json --ref-file='../t/reporter/reference.fasta' --ref-sequencer='Illumina' --ref-source='NCBI' --plasmids='YES' --genus='Escherichia' --species='coli' --serotype='O157'`;
#is_valid_json($output, "The reference stats are being generated and formatted properly.");

$output = `perl $script_dir/../lib/reporter.pl --step=record_file_sizes --output=reporter.json --json=reporter.json --file-type='bam' --file-sizes file1=../t/reporter/sample1.bam --file-sizes file2=../t/reporter/sample2.bam --file-sizes file3=../t/reporter/sample3.bam`;
...
$output = `perl $script_dir/../lib/reporter.pl --step=vcf2core_stats --output=reporter.json --json=reporter.json --vcf2core-stats=../t/reporter/vcf2core.out`;
